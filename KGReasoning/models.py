...
class BoxOffsetIntersection(nn.Module):
    ...
    def forward(self, embeddings):
        ...
        sigmoid = nn.Sigmoid()
        gate = sigmoid(self.layer2(layer1_mean))
...
class CenterIntersection(nn.Module):
    ...
    def forward(self, embeddings):
        layer1_act = F.relu(self.layer1(embeddings)) # (num_conj, batchsize, dim)
        attention = F.softmax(self.layer2(layer1_act), dim=0) # (num_conj, batchsize, dim)
        embedding = torch.sum(attention * embeddings, dim=0)

        return embedding

class CenterNegation(nn.Module):
    def __init__(self, alpha):
        super(CenterNegation, self).__init__()
        self.alpha = alpha
        # self.dim = dim
        # # self.layer_pos = nn.Linear(self.dim, self.dim)
        # # self.layer_inter = nn.Linear(self.dim, self.dim)
        # self.layer_1 = nn.Linear(self.dim, self.dim)
        # self.layer_2 = nn.Linear(self.dim, self.dim)
        
        # nn.init.xavier_uniform_(self.layer_pos.weight)
        # nn.init.xavier_uniform_(self.layer_inter.weight)
        # nn.init.xavier_uniform_(self.layer_1.weight)
        # nn.init.xavier_uniform_(self.layer_2.weight)
    
    def forward(self, pos_embedding, inter_embedding, inter_offset):
        """
        pos_embedding: (batchsize, dim)
        inter_embedding: (batchsize, dim)
        ---------------------------------
        embedding: (batchsize, dim)
        """
        embedding = pos_embedding + torch.sign(pos_embedding - inter_embedding) * inter_offset * self.alpha / 2 
        # pos_act = F.relu(self.layer_pos(pos_embedding)) # (batchsize, dim)
        # inter_act = F.relu(self.layer_inter(inter_embedding)) # (batchsize, dim)
        # act1 = torch.stack([pos_act, inter_act]) # (2, batchsize, dim)
        # act1 = F.relu(self.layer_1(torch.stack([pos_embedding, inter_embedding])))
        # attention = F.softmax(self.layer_2(act1), dim=0)[0,:,:] # (batchsize, dim)
        # lambda_center = 1 / (1 - attention)
        # embedding = lambda_center * pos_embedding + (1 - lambda_center) * inter_embedding # lambda > 1
        return embedding
    
class OffsetNegation(nn.Module):
    def __init__(self, alpha):
        super(OffsetNegation, self).__init__()
        self.alpha = alpha
        # self.dim = dim
        # # self.layer_pos = nn.Linear(self.dim, self.dim)
        # # self.layer_inter = nn.Linear(self.dim, self.dim)
    ...
    def forward(self, pos_offset, inter_offset):
        # pos_act = F.relu(self.layer_pos(pos_offset)) # (batchsize, dim)
        # inter_act = F.relu(self.layer_inter(inter_offset)) # (batchsize, dim)
        # act1 = F.relu(self.layer_1(torch.stack([pos_offset, inter_offset])))
        # lambda_offset = F.softmax(self.layer_2(act1), dim=0)[0,:,:] # (batchsize, dim)
        # offset = pos_offset - lambda_offset * inter_offset
        offset = pos_offset - inter_offset * self.alpha
        return offset
...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma, 
        ...
        if self.geo == 'box':
            ...
            self.center_negation = CenterNegation(self.args.alpha)
            self.offset_negation = OffsetNegation(self.args.alpha)
        elif self.geo == 'vec':
            self.center_net = CenterIntersection(self.entity_dim)
        elif self.geo == 'beta':
    ...
    def embed_query_box(self, queries, query_structure, idx):
        ...
        if all_relation_flag:
            # If 1p, 2p and 3p: (e,(r,)), (e,(r, r)), and (e, (r, r, r))
            if query_structure[0] == 'e':
                # select corresponding embedding of entities. idx is a variable to show which elements in queries are entities. Here, queries[:, idx] is all index of entities.
            ...
            """This part deal with projection."""
            # for each 'r' in the 1p, 2p, or 3p path.
            for i in range(len(query_structure[-1])):
                if query_structure[-1][i] == 'n':
                    """deal with negation after this part"""
                    pass
                else:
                    r_embedding = torch.index_select(self.relation_embedding, dim=0, index=queries[:, idx])
                    r_offset_embedding = torch.index_select(self.offset_embedding, dim=0, index=queries[:, idx])
                    embedding = embedding + r_embedding
                    offset_embedding = offset_embedding + self.func(r_offset_embedding)
                idx += 1
        else: # For example, 2i or 2in
            embedding_list = []
            offset_embedding_list = []
            num_branch = len(query_structure)
            index_negative = set() # no negative part
            for i, branch in enumerate(query_structure):
                relation_path = branch[-1]
                if relation_path[-1] == 'n':
                    index_negative.add(i)
                    
            if index_negative:
                index_positive = set(range(num_branch)) - index_negative
                """ deal with negative part. """
                assert len(index_negative) == 1
                query_neg_part = query_structure[list(index_negative)[0]]
                query_pos_part = tuple(query_structure[i] for i in index_positive)
                assert query_pos_part
                if min(index_positive) > max(index_negative):
                    neg_embedding, neg_offset, idx = self.embed_query_box(queries, query_neg_part, idx)
                    pos_embedding, pos_offset, idx = self.embed_query_box(queries, query_pos_part, idx)

                elif min(index_negative) > max(index_positive):
                    pos_embedding, pos_offset, idx = self.embed_query_box(queries, query_pos_part, idx)
                    neg_embedding, neg_offset, idx = self.embed_query_box(queries, query_neg_part, idx)
                else:
                    assert False, "positive part and negative part should be devided."
                inter_embedding = self.center_net(torch.stack([pos_embedding, neg_embedding]))
                inter_offset = self.offset_net(torch.stack([pos_offset, neg_offset]))
                embedding = self.center_negation(pos_embedding, inter_embedding, inter_offset) # center of negation
                offset_embedding = self.offset_negation(pos_offset, inter_offset) # offset of negation                
            else:    
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                for idx, (i, query, query_structure) in enumerate(zip(argsort[:, 0], queries_unflatten, query_structures)):
                    ...
                    log = np.log(2) * torch.mean(1./torch.log(cur_ranking+1)).item()
                    sqrt = torch.mean(1./torch.sqrt(cur_ranking)).item()
                    p2_3 = torch.mean(1./torch.float_power(cur_ranking, 2/3)).item()
                    p1_3 = torch.mean(1./torch.float_power(cur_ranking, 1/3)).item()
                    p1_4 = torch.mean(1./torch.float_power(cur_ranking, 1/4)).item()
                    p3_4 = torch.mean(1./torch.float_power(cur_ranking, 3/4)).item()
