...
def parse_args(args=None):
    ...
    parser.add_argument('--sparse_test', action='store_true', help='test different sparsity of test set')
    ...
    parser.add_argument('--save_checkpoint_steps', default=2000, type=int, help="save checkpoints every xx steps")
    parser.add_argument('--valid_steps', default=30000, type=int, help="evaluate validation queries every xx steps")
    parser.add_argument('--log_steps', default=100, type=int, help='train log every xx steps')
    parser.add_argument('--test_log_steps', default=5000, type=int, help='valid/test log every xx steps')
    
    parser.add_argument('--nentity', type=int, default=0, help='DO NOT MANUALLY SET')
    parser.add_argument('--nrelation', type=int, default=0, help='DO NOT MANUALLY SET')
    
    parser.add_argument('--label', type=str, default='0', help='label')
    ...
    parser.add_argument('--pretrain_path', default=None, type=str, help='path for loading pre-train model')
    ...
    parser.add_argument('--no_valid_train', action='store_true', help='pass training set when validating')
    parser.add_argument('--only_train_negation', action='store_true', help='only train negation')
...
def load_data(args, tasks):
    ...
    valid_queries = valid_hard_answers = valid_easy_answers = None
    test_queries = test_hard_answers = test_easy_answers = None
    if args.do_train:
    ...
    if args.do_test:
        ...
        test_hard_answers = pickle.load(open(os.path.join(args.data_path, "test-full-hard-answers.pkl"), 'rb'))
        test_easy_answers = pickle.load(open(os.path.join(args.data_path, "test-easy-answers.pkl"), 'rb'))
        if args.sparse_test:
            assert args.tasks == '1p'
            # sparse_test_hard_answers = dict()
            # listdir = os.listdir(args.data_path)
            # sparse_file = filter((lambda x: 'test-hard-answers-' in x and '.pkl' in x), listdir)
            # for file in sparse_file:
            #     with open(os.path.join(args.data_path, file), 'rb') as f:
            #         sparse_test_hard_answers[int(file[18:][:-4])] = pickle.load(f)
            test_sparse_hard_answers = pickle.load(open(os.path.join(args.data_path, "test-sparse-hard-answers.pkl"), 'rb'))
    ...
    for name in all_tasks:
        ...
        if name not in tasks or evaluate_union != args.evaluate_union:
            ...
            if args.do_train and query_structure in train_queries:
                del train_queries[query_structure]
            if args.do_valid and query_structure in valid_queries:
                del valid_queries[query_structure]
            if args.do_test and query_structure in test_queries:
                del test_queries[query_structure]
    if args.sparse_test:
        return train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, test_sparse_hard_answers
    else:
...
def main(args):
    ...
    for task in tasks:
        if 'n' in task and args.geo in ['vec']:
            assert False, "GQE cannot handle queries with negation"
    ...
    args.save_path = os.path.join(prefix, args.data_path.split('/')[-1], args.geo)
    if args.geo in ['box']:
    ...
    if args.checkpoint_path is not None:
        ...
        args.save_path = os.path.join(args.save_path, args.label)
    ...
    if args.sparse_test:
        train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, sparse_test_hard_answers = load_data(args, tasks)   
    else:
    ...
    no_answers = defaultdict(set) # used as train_easy_answers
    ...
    if args.do_test:
        ...
        # if args.sparse_test:
        #     sparse_test_dataloader = dict()
        #     for d in sparse_test_hard_answers:
        #         sparse_test_dataloader[d] = DataLoader(
        #             TestDataset(
        #                 test_queries,
        #                 args.nentity,
        #                 args.nrelation,
    ...
    if args.only_train_negation:
        for name, param in model.named_parameters():
            if 'negation' not in name and 'embedding' not in name:
    ...
    if args.pretrain_path:
        logging.info('Loading pretrain model {:s}'.format(args.pretrain_path))
        pretrain_model = torch.load(os.path.join(args.pretrain_path, 'checkpoint'))
        missing_keys, unexpected_keys = model.load_state_dict(pretrain_model['model_state_dict'], strict=False)
        if missing_keys:
            logging.info('Missing keys: {}'.format(missing_keys))
        if unexpected_keys:
            logging.info('unexpexted keys: {}'.format(unexpected_keys))
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.valid_steps == 0 and step > 0:
                if not args.no_valid_train:
                    logging.info('Evaluating on Training Dataset...')
                    train_all_metrics = evaluate(model, no_answers, train_answers, args, train_dataloader, query_name_dict, 'Training', step, writer)
                if args.do_valid:
                ...
                if args.do_test:
                    ...
                    if args.sparse_test:
                        logging.info('Evaluating on Sparse Test Dataset...')
                        test_all_metrics = evaluate(model, test_easy_answers, sparse_test_hard_answers, args, test_dataloader, query_name_dict, 'Test_sparse', step, writer)
